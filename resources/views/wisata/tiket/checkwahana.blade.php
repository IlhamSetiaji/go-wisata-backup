...
<div class="row">
    <div class="col-12 col-md-6 col-lg-6">
        <form action="{{ route('checkwahana.cek') }}" method="post">
            @csrf
            <div class="form-group">
                <h6>Scan QR Code Tiket</h6>
                <div class="input-group">
                    <input type="text" name="order_id" id="text" readonly="" placeholder="Scan QR Code"
                        class="form-control">
                    ...
                    <table class="table table-hover" id="camp">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(App\Models\Camp::where('kode_camp',$dtc->alat_id )->pluck('kategori')->first()
                                    == "alat" )
                                    @if(!$datacamp->alat_id == null)
                                    ...
                                    @else
                                    <form action="{{ url('wisata/backcamp',[$id])}}" method="POST"
                        ...
                        <table class="table table-borderless">
                            <tr>
                                <td class="col-8">
                                    ...
                                    Berhasil Dibayar
                                    &nbsp;
                                    <a href="{{route('update.kedatangan',[$id])}}"><button class="btn btn-warning">
                                            Konfirmasi Datang</button></a>
                                    @else
                                    @if(!$datacamp->alat_id == null)
                                    @if ($datacamp->status == 1)
                                    {{ $datacamp->tgl_kembaliin }}
                                    <i class="fas fa-check-double"></i> <button class="btn btn-primary" disabled> Sudah
                                        Selesai</button>
                                    @else
                                    @endif
                                    @endif
                        ...
                        <table class="table table-borderless">
                            <tr>
                                <td class="col-8">
                                    ...
                                    {{-- {{ dd($data) }} --}}
                                    @if($datawahana->kedatangan == 1 && $data->count == 0 && $data->type_bayar == 'Transfer')
                                    Sudah Dipakai
                                    @else
